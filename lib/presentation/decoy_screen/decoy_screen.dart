...
import 'package:shared_preferences/shared_preferences.dart'; // Added for failed attempts
...
class _DecoyScreenState extends ConsumerState<DecoyScreen> {
  ...
  static const String _failedAttemptsKey = 'failed_login_attempts_conduit'; // Made key more specific
  static const int _maxFailedAttempts = 5;

  @override
  void initState() {
    super.initState();
    _loadFailedAttempts(); // Load failed attempts count
  ...
  void _handleTap() {
    if (widget.isPostDestruct || !_systemCheckComplete || _failedLoginAttempts >= _maxFailedAttempts) { // Prevent dialog if already maxed out
      return;
    }
    setState(() {
      _tapCount++;
  ...
  Future<void> _triggerSelfDestruct({String triggeredBy = "Unknown"}) async {
    ...
    final currentRef = ref; // Capture ref before async gap
    ...
    await currentRef.read(selfDestructServiceProvider).initiateSelfDestruct(currentContext, triggeredBy: triggeredBy, performLogout: true);
  }
              ...
              children: <Widget>[
                ...
                if (_systemCheckComplete && !widget.isPostDestruct)
                  Padding(
                    ...
                    child: Text(
                      "النظام جاهز. انقر 5 مرات للوصول المشفر.",
                      textAlign: TextAlign.center,
