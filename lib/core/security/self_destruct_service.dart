...
// import "package:sembast/sembast_io.dart"; // Removed unused import
...
class SelfDestructService {
  ...
  Future<void> _secureWipeSembastDatabase(LoggerService logger) async {
    // Changed AppLogger to LoggerService
    try {
  ...
  Future<void> initiateSelfDestruct(BuildContext context,
      {String? triggeredBy, bool performLogout = true}) async {
    ...
    logger.error(
        "FULL SELF-DESTRUCT SEQUENCE INITIATED by: ${triggeredBy ?? 'Unknown'}. Agent: $currentAgentCode. Perform Logout: $performLogout.",
        "SELF_DESTRUCT_SERVICE");

    // Capture BuildContext and mounted state before async gaps
    final scaffoldMessenger = ScaffoldMessenger.of(context);
    final navigator = Navigator.of(context);
    bool isContextMounted() =>
        context.mounted; // Helper to check mounted status
